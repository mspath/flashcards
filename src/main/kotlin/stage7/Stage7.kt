...
/**
 * helper method to put quotes around a String
 */
fun String.putInQuotes() = """"${this}""""

// crude implementation just to get it working
fun parseArgs(args: Array<String>): Pair<String?, String?> {
    ...
    if (args.size > 1 && args[0] == "-import") {
        import = args[1]
    } else if (args.size > 3 && args[2] == "-import") {
        import = args[3]
    }
    if (args.size > 1 && args[0] == "-export") {
        export = args[1]
    } else if (args.size > 3 && args[2] == "-export") {
        export = args[3]
    }
    return Pair(import, export)
}

data class Flashcard(var card: String = "", var definition: String = "", var mistakes: Int = 0) {
...
fun loop(importExport: Pair<String?, String?>) {
    importExport.first?.let {
        Cards.importFromFile(it)
    }
    while (true) {
        ...
        when (action) {
            ...
            "exit" -> {
                println("Bye bye!")
                importExport.second?.let {
                    Cards.exportToFile(it)
...
/**
 * overriding to both print to stdout and log the String
 */
